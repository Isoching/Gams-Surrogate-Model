...
*eq1(i).. x(n,i) =e= xo(n,i) ;

eq2(n,j)..   yt(n,j) =e= c+sum((i,k),a(i,k)*power(xo(n,i),ord(k)))
                            + sum((i,ip,k,kp)$(ord(i)<ord(ip)),b(i,ip,k,kp)*power(xo(n,i),ord(k))*power(xo(n,ip),ord(kp)));

eq3..             obj =e= sum((n,j),power((yt(n,j)-y(n,j)),2))/749  ;
eq4(n)..          loss(n) =e= sum(j,power((yt(n,j)-y(n,j)),2))  ;

model case1 /all/;
solve case1 using nlp min obj;
display obj.l, yt.l, y, loss.l

execute_unload "yt.gdx" yt.l;
execute 'gdxxrw.exe yt.gdx o=D:\peaks\HDMROutput\val14.xlsx var=yt.l rng=yt!';
*execute_unload "y.gdx" y;
*execute 'gdxxrw.exe y.gdx o=D:\peaks\HDMROutput\val14.xlsx var=y.l rng=y!';
execute_unload "loss.gdx" loss.l;
execute 'gdxxrw.exe loss.gdx o=D:\peaks\HDMROutput\val14.xlsx var=loss.l rng=loss!';